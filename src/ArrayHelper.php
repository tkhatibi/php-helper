...
class ArrayHelper{
     ...
     * ```
     *
     * The result of `ArrayHelper::toArrayHelper($post, $properties)` could be like the following:
     *
     * ```php
     * [
     *     'id' => 123,
     ...
     */
    public static function toArrayHelper($object, $properties = [], $recursive = true)
    {
        if (is_array($object)) {
            if ($recursive) {
                foreach ($object as $key => $value) {
                    if (is_array($value) || is_object($value)) {
                        $object[$key] = static::toArrayHelper($value, $properties, true);
                    }
                }
            }
            return $object;
            ...
            if (!empty($properties)) {
                ...
                if (!empty($properties[$className])) {
                    ...
                    return $recursive ? static::toArrayHelper($result, $properties) : $result;
                }
            }
            if ($object instanceof ArrayHelperable) {
                $result = $object->toArrayHelper([], [], $recursive);
            } else {
                $result = [];
                foreach ($object as $key => $value) {
                    $result[$key] = $value;
    ...
    public static function index($array, $key, $groups = [])
    {
        ...
        foreach ($array as $element) {
            $lastArrayHelper = &$result;
            foreach ($groups as $group) {
                $value = static::getValue($element, $group);
                if (!array_key_exists($value, $lastArrayHelper)) {
                    $lastArrayHelper[$value] = [];
                }
                $lastArrayHelper = &$lastArrayHelper[$value];
            }
            if ($key === null) {
                if (!empty($groups)) {
                    $lastArrayHelper[] = $element;
                }
            } else {
                ...
                if ($value !== null) {
                    ...
                    $lastArrayHelper[$value] = $element;
                }
            }
            unset($lastArrayHelper);
        }
        return $result;
    }
    /**
